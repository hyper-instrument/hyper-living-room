...
| `set_plug_power` | `on`(bool) | Tapo 插座**市电**硬开关。插座平时**一直开着，只用来统计电量**；正常控制空调一律用 `set_ac`，几乎不需要动它 |
...
- **插座只是电表**：Tapo 插座平时一直开着，只用来读功率/用电量。**所有开关/调节都走红外 `set_ac`**，不要用插座来开关空调。只有用户明确说「断电 / 拔电 / 彻底关掉插座」时才用 `set_plug_power`。
- **区分两种「关」**：
...
- 红外是**单向**的：`get_status` 里的 `ac_ir_*` 是「最后一次发送的指令」，不是空调实测状态。发完指令要说「已发送」，不要说「已开启/已关闭」。
- **用插座功率确认空调是否真的执行了**（推荐流程）：发送 `set_ac` 前先记下 `get_status` 的 `power_w`；发送后等 20–60 秒再查一次。开机成功→功率会从待机（<5W）明显上升（几十到几百 W）；关机成功→功率降回待机。功率没变化 = 空调很可能没收到红外（提醒用户检查 Stick 是否对准空调）。
